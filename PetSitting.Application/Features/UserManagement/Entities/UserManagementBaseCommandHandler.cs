...
namespace PetSitting.Application.Features.UserManagement.Entities
{
    ...
    public abstract class UserManagementBaseCommandHandler<TRequest, TResponse, TValidator> : IRequestHandler<TRequest, TResponse>
    ...
    public abstract class UserManagementBaseCommandHandler<TRequest, TResponse> : IRequestHandler<TRequest, TResponse>
        where TRequest : IRequest<TResponse>
        where TResponse : BaseResponse, new()
    {
        public async Task<TResponse> Handle(TRequest request, CancellationToken cancellationToken)
